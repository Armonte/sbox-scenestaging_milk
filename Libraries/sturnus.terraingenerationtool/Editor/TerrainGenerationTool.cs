...
public class TerrainGenerationTool : Widget
{
	...
	TexturePreview PreviewImage;
	Texture _preview_splatmap_texture;
	TexturePreview PreviewSplatmap;

	class TexturePreview : Widget
	{
		Pixmap _pixmap;
		Texture _texture;
		public Texture Texture
		{
			...
			set
			{
				_texture = value;
				_pixmap = null;
				if ( _texture.IsValid() && _texture.Width > 0 && _texture.Height > 0 )
				{
					try { _pixmap = Pixmap.FromTexture( _texture, true ); }
					catch { _pixmap = null; }
				}
				Update();
			}
		...
		public void LoadFromFile( string absolutePath )
		{
			...
			{
				try { _pixmap = Pixmap.FromFile( absolutePath ); }
				catch { _pixmap = null; }
			}
			Update();
		}
		public TexturePreview( Widget parent ) : base( parent ) { }
		protected override void OnPaint()
		{
			if ( _pixmap == null ) return;
			Paint.Draw( LocalRect, _pixmap, 1f, 0f );
		}
	}
	...
	public TerrainGenerationTool( Widget parent ) : base( parent, false )
	{
		...
		var _image_preview = new TexturePreview( this );
		...
		var _splatmap_preview = new TexturePreview( this );
		...
		GenerateButton.Clicked += () =>
		{
			...
			string preview_image_abs = Path.Combine( GenerationPath, $"TerrainGenerationUtility_preview.png" );
			PreviewImage.LoadFromFile( preview_image_abs );

			string preview_splatmap_abs = Path.Combine( GenerationPath, $"TerrainGenerationUtility_splat_preview.png" );
			PreviewSplatmap.LoadFromFile( preview_splatmap_abs );
